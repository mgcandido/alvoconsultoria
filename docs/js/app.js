  ...
  switch (tipo) {
    ...
    case 'ncm-ibs':
      ...
      columns = [
        ...
        { key: 'cstNovo', label: 'CST', type: 'code' },
        { key: tipo === 'ncm-cbs' ? 'novoCBS' : 'novoIBS', label: 'Situação', type: 'badge' },
    ...
    case 'transicao-pis-cofins':
      ...
      columns = [
        ...
        { key: 'descricaoAntiga', label: 'Descrição Antiga' },
        { key: 'cstNovo', label: 'CST Novo', type: 'code' },
        { key: 'novoCBS', label: 'CBS', type: 'badge' },
        { key: 'novoIBS', label: 'IBS', type: 'badge' },
        { key: 'observacoes', label: 'cClassTrib / Observações' }
      ];
      break;
    case 'cclasstrib':
      data = TABELA_CCLASSTRIB;
      columns = [
        { key: 'cst', label: 'CST', type: 'code' },
        { key: 'codigo', label: 'cClassTrib', type: 'code' },
        { key: 'nome', label: 'Nome' },
        { key: 'descricao', label: 'Aplicação (LC 214/2025)' },
...
function getBadgeClass(value) {
  ...
  if (str.includes('isent') || str.includes('imun')) return 'badge-isento';
  if (str.includes('zero')) return 'badge-zero';
  if (str.includes('reduzid')) return 'badge-reduzido';
  if (str.includes('monofásico') || str.includes('monofasico') || str.includes('substituição') || str.includes('substituicao')) return 'badge-st';
  if (str.includes('diferimento') || str.includes('suspensão') || str.includes('suspensao')) return 'badge-default';
  if (str.includes('análise') || str.includes('analise')) return 'badge-default';
